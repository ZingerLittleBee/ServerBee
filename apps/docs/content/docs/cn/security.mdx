...
在 `server.toml` 中添加 OAuth 配置（详见 [Server 配置](/cn/docs/server)）：
...
首次启动（数据库中没有任何用户）时，ServerBee 会自动创建管理员账号，随机生成密码，并以醒目的凭据横幅在 Server/容器日志中打印一次。无法通过环境变量预设用户名或密码。首次登录时你将被要求修改此密码，并可在此时选择一个新的用户名。
