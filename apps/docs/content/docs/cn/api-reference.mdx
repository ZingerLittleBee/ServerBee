...
| POST | `/api/agent/register` | Agent 使用一次性注册码注册（注册码在首次成功注册时被消费） |
...
| Agent 注册码 | `POST /api/agent/enrollments`、`GET /api/agent/enrollments`、`DELETE /api/agent/enrollments/{id}`、`POST /api/agent/{id}/rotate-token` |
...
以下端点均需 Admin 角色。所有响应遵循统一的 `{ "data": ... }` 包装格式。

### `POST /api/agent/enrollments` —— 生成注册码

生成一个一次性、短时有效的注册码（默认 TTL 600 秒 / 10 分钟，单次使用，在 Agent 首次成功注册时被消费）。
...
明文 `code` 只在生成时返回一次，请立即复制。

### `GET /api/agent/enrollments` —— 列出注册码

列出已生成的注册码及其元信息。出于安全考虑，**永不返回明文注册码**，仅返回 8 位前缀和状态/过期等元数据。
...
### `DELETE /api/agent/enrollments/{id}` —— 删除注册码

删除一个尚未使用的注册码，使其立即失效。

### `POST /api/agent/{id}/rotate-token` —— 轮换/吊销 run token

轮换（吊销）指定服务器的 Agent run token。旧 token 立即失效，对应 Agent 会被强制断开并需使用新 token 重新连接。
