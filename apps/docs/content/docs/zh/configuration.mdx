...
| `SERVERBEE_IP_QUALITY__IPAPI_IS__ENDPOINT` | `https://api.ipapi.is` | 自建镜像或测试时覆盖。 |
...
| `endpoint` | string | `"https://api.ipapi.is"` | 自建镜像或测试时覆盖。 |
...
| 键 | 类型 | 默认值 | 说明 |
|----|------|--------|------|
| `enabled` | bool | `false` | 启用文件管理能力。Server 端还需为该 Agent 启用 `CAP_FILE` |
