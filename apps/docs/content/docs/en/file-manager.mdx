...
Both layers must be satisfied before the agent advertises the capability: if `file` is allowed but the subsystem is disabled (or `root_paths` is empty), the agent drops `file` from its reported capabilities at startup, so the server and UI never show a File Manager that cannot serve requests.
