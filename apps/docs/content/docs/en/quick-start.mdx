...
Sign in to the dashboard as an admin, go to **Servers → Add Server**, enter a name for the new machine, click **Generate code**, and copy the one-time enrollment code. This also creates a pending server entry that the agent will claim when it first connects.
