...
<Callout type="warn">
The published `v1.0.0-beta.1` dashboard has a known Add Server crash on remote plain-HTTP origins because that build calls `crypto.randomUUID()` directly. Until a newer release containing the fallback is published, use the domain/HTTPS path below (or build the current `main` source) rather than `http://<server-ip>:9527`. A successful `/healthz` or login does not prove the affected Servers page works.
</Callout>
...
### Step 1: Add the Server and receive its enrollment offer
