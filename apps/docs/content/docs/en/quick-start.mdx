---
title: Quick Start
description: Get the ServerBee server and agents running in minutes with the one-line install script.
icon: Zap
---
...
A single command (handy for scripted deploys):

```bash
curl -fsSL https://raw.githubusercontent.com/ZingerLittleBee/ServerBee/main/deploy/install.sh | sudo bash -s -- server -y
```

For the interactive wizard (choose language, binary or Docker), run it with no arguments:
...
7. Writes `auth.secure_cookie = false` so browser login works over plain HTTP
...
1. Open `http://your-server-ip:9527` in your browser
...
If you already have a domain whose DNS `A` record points to this server, the script can set up Caddy and issue an HTTPS certificate for you:
...
<Callout type="warn">
Once you're on HTTPS, set `auth.secure_cookie` back to `true` in `/opt/serverbee/etc/server.toml`, restart the server, and update the `server_url` of any agents already connected to the new `https://` address.
</Callout>
