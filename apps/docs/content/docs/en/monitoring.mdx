...
ServerBee supports real-time Docker container monitoring when the agent has access to the Docker daemon. This feature requires the `docker` capability (`CAP_DOCKER`), which is off by default. Capabilities are agent-owned — enable it on the agent host by adding `docker` to its `[capabilities]` allow list (or `--allow-cap docker`) and restarting the agent. See [Capabilities](/en/docs/capabilities).
