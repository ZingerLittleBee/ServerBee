...
On Linux, the agent reads `/proc/diskstats` to collect per-disk read/write throughput:

| Metric | Unit | Description |
|--------|------|-------------|
| Read Speed | bytes/sec | Read throughput per physical disk |
| Write Speed | bytes/sec | Write throughput per physical disk |

Only physical block devices are tracked (e.g., `sda`, `nvme0n1`). Virtual devices (`loop*`, `dm-*`, `ram*`, `sr*`) and partitions (`sda1`, `nvme0n1p1`) are excluded. On the first sample after agent startup, a baseline is established and an empty list is reported. Subsequent samples compute delta-based rates.
...
On non-Linux platforms, disk I/O collection is not available and `disk_io` is reported as `null`. The server and frontend handle this gracefully.
...
- Disk I/O charts with merged and per-disk views (Linux, historical mode)
...
When historical disk I/O data is available (Linux agents only), the server detail page displays a Disk I/O chart with two views:
