...
max_servers = 0                # Soft limit for newly auto-registered servers
...
The full key is printed to the logs on first startup and can also be viewed or regenerated later from **Settings** in the web UI. Regeneration affects only future registrations. Already connected agents keep using their stored tokens.
