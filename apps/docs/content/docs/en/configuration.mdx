...
| `trusted_proxies` | string[] | `[]` | CIDR ranges of trusted reverse proxies. When set, the real client IP is extracted from `X-Forwarded-For` or `X-Real-IP` headers only for requests originating from these ranges |
