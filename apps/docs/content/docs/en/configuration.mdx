...
Configure the optional third-party IP risk scoring provider. If no provider is configured, the server uses only the local GeoIP database for IP metadata. See [IP Quality](/en/docs/ip-quality) for feature details.

<Callout type="warn">
`ip-api` (ip-api.com) uses an HTTP-only endpoint and its free tier is restricted to non-commercial use only. Review its [terms of service](https://ip-api.com/docs/legal) before using it in production.
</Callout>

| Environment Variable | Default | Description |
|---------------------|---------|-------------|
| `SERVERBEE_IP_QUALITY__RISK_PROVIDER` | `none` | Active risk provider: `none`, `scamalytics`, `ipqs`, `proxycheck`, `abuseipdb`, or `ip-api` |
| `SERVERBEE_IP_QUALITY__SCAMALYTICS__API_KEY` | -- | Scamalytics API key. Required when `risk_provider = "scamalytics"` |
| `SERVERBEE_IP_QUALITY__SCAMALYTICS__ENDPOINT` | `""` | Scamalytics account-bound API subdomain (e.g. `api1`). Leave empty for default |
| `SERVERBEE_IP_QUALITY__IPQS__API_KEY` | -- | IPQualityScore API key. Required when `risk_provider = "ipqs"` |
| `SERVERBEE_IP_QUALITY__IPQS__ENDPOINT` | `""` | IPQualityScore endpoint override. Leave empty for default |
| `SERVERBEE_IP_QUALITY__PROXYCHECK__API_KEY` | -- | proxycheck.io API key. Required when `risk_provider = "proxycheck"` |
| `SERVERBEE_IP_QUALITY__PROXYCHECK__ENDPOINT` | `""` | proxycheck.io endpoint override. Leave empty for default |
| `SERVERBEE_IP_QUALITY__ABUSEIPDB__API_KEY` | -- | AbuseIPDB API key. Required when `risk_provider = "abuseipdb"` |
| `SERVERBEE_IP_QUALITY__ABUSEIPDB__ENDPOINT` | `""` | AbuseIPDB endpoint override. Leave empty for default |
...
Optional third-party IP risk scoring. Baseline IP metadata (country, ASN, IP type) is always derived from the local GeoIP MMDB and does not require a provider. See [IP Quality](/en/docs/ip-quality) for feature details.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `risk_provider` | string | `"none"` | Active risk provider: `none`, `scamalytics`, `ipqs`, `proxycheck`, `abuseipdb`, or `ip-api` |

### `[ip_quality.scamalytics]` -- Scamalytics

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `api_key` | string | -- | Scamalytics API key |
| `endpoint` | string | `""` | Account-bound API subdomain (e.g. `api1`, `api11`). Leave empty to use the Scamalytics default |

### `[ip_quality.ipqs]` -- IPQualityScore

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `api_key` | string | -- | IPQualityScore API key |
| `endpoint` | string | `""` | Endpoint override. Leave empty to use the default |

### `[ip_quality.proxycheck]` -- proxycheck.io

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `api_key` | string | -- | proxycheck.io API key |
| `endpoint` | string | `""` | Endpoint override. Leave empty to use the default |

### `[ip_quality.abuseipdb]` -- AbuseIPDB

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `api_key` | string | -- | AbuseIPDB API key |
| `endpoint` | string | `""` | Endpoint override. Leave empty to use the default |
