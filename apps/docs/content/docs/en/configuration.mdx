...
| `SERVERBEE_FEATURE__SPA_THEMES` | `true` | Set `feature.spa_themes` to false to disable custom frontend theme upload and activation. Any active custom frontend falls back to the default SPA at serve time; stored packages are preserved. See [Custom Frontend Themes](/en/docs/custom-frontend). If a custom frontend is active and you need to recover the default UI, visit `?theme=default` in your browser |
