...
<Callout type="info">
The dashboard "latest version" check uses the Server-configured release source (`upgrade.release_base_url` / `upgrade.latest_version_url`). For consistent upgrade behavior, point both the Server and the Agent at the same release repository unless you intentionally want them to track different sources.
</Callout>
