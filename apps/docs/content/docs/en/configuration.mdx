...
| `SERVERBEE_FILE__ROOT_PATHS` | `[]` | Allowed root paths (comma-separated, e.g. `/home,/var/log`). Empty means filesystem root |
...
| `root_paths` | string[] | `[]` | Restrict browsing to these directories. Empty array means full filesystem access |
