...
Configuration values are merged from multiple sources. Later sources override earlier ones, so environment variables always win:
...
Every TOML key maps directly to an environment variable: prefix with `SERVERBEE_`, uppercase the key, and replace each level of nesting with `__` (double underscore). For example, `auth.secure_cookie` becomes `SERVERBEE_AUTH__SECURE_COOKIE`.
