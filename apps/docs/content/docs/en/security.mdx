...
On first start (when no users exist) ServerBee auto-creates an admin account with a randomly generated password and prints it once to the server/container logs as a highlighted credentials banner. There is no way to preset the username or password via environment variables. On first login you are required to change this password, and may optionally choose a different username at that time.
