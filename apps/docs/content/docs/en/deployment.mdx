...
The template pulls `ghcr.io/zingerlittlebee/serverbee-server:latest` by default, which only follows stable releases. Pre-release tags (e.g. `1.0.0-alpha.1`) are published to GHCR but do **not** move `:latest`.
...
Save and trigger a redeploy. Remove the variable (or set it back to `latest`) to return to the stable channel.

<Callout type="info">
The Dockerfile exposes this switch via `ARG SERVERBEE_IMAGE_TAG=latest`. Railway injects service variables as both build args and runtime environment variables; since this variable is not part of the `SERVERBEE_*` configuration schema, it has no runtime side effects.
</Callout>
...
Stable releases are selected by default. For beta testing, pass `--channel beta` to install or upgrade the newest published prerelease, or use `--version vX.Y.Z-beta.N` to pin an exact release. The CLI refuses to replace an installed version with an older SemVer.
...
| Agent RAM | 10 MB (cold) | 30–40 MB (steady, 8h+) |
