...
The bootstrap script uses the same management logic shipped in `deploy/serverbee.sh`, so ongoing operations use the same command set:
...
Use `deploy/serverbee.sh` directly when you cloned the repo, or use the curl bootstrap flow on fresh hosts that do not have the repository checked out.
