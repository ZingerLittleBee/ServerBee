...
- Low-risk capabilities (`ping_icmp`, `ping_tcp`, `ping_http`) are enabled by default
- High-risk capabilities (`terminal`, `exec`, `upgrade`, `file`, `docker`) are disabled by default
