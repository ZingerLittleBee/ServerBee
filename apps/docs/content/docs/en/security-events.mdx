...
Agents detect host-level intrusion signals and stream structured events to the server. Raw logs never leave the host — only the parsed event metadata is reported. Events are browsable in the UI and can drive notifications through the standard alert pipeline.
