...
ServerBee defines 10 capability bits, divided into two risk levels. The valid mask is `1023` (bits 0..=9).
...
Newly registered agents default to a capabilities value of `316` (auto upgrade + all three ping capabilities + security events).
...
- `316` = Auto Upgrade + ICMP + TCP + HTTP + Security Events (default)
- `1023` = all capabilities enabled (full valid mask, bits 0..=9)
- `444` = default + Docker
- `380` = default + File Manager
- `828` = default + Firewall Blocklist
- `60` = legacy default (no Security Events)
