...
ServerBee defines 11 capability bits, divided into two risk levels. The valid mask is `2047` (bits 0..=10).
...
Newly registered agents default to a capabilities value of `1852` (auto upgrade + three ping capabilities + security events + firewall blocklist + IP quality).
...
- `60` = legacy default (no Security Events / firewall / IP quality)
