...
Service Monitors are synthetic checks that run from the central ServerBee server. They let you monitor public-facing services even when those services are not running on a ServerBee agent host.
