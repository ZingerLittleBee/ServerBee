...
For comparison: among comparable resident monitoring agents, Netdata often reaches tens to hundreds of MB, Telegraf ~30–50 MB, and node_exporter ~15–20 MB. The ServerBee Agent's steady-state ~27 MB memory + &lt;1% CPU keeps it firmly in the lightweight tier.
...
<Callout type="warn">
`MALLOC_ARENA_MAX` only affects glibc. The official Linux release binaries and Docker images are static musl builds, where it has no effect. `v1.0.0-beta.1` release binaries were linked against an allocator that never reused memory freed on another thread, so the Server grew about 20 MB per day even while idle. Upgrade to `v1.0.0-beta.2` or later.
</Callout>
