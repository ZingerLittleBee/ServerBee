...
The Agent is a lightweight probe with a very small resident footprint.

| Resource | Measured | Notes |
|----------|----------|-------|
| Memory (systemd cgroup, real private memory) | ~4.3 MB, peak ~5.1 MB | Most accurate memory figure |
| Memory (`ps` RSS, includes shared library mappings) | ~10–11 MB | Overestimate, includes shared libs |
| CPU | ~0.7% average (single core) | Includes the per-3s collection spike |
| Threads | 10 | tokio runtime |
...
For comparison: among comparable resident monitoring agents, Netdata often reaches tens to hundreds of MB, Telegraf ~30–50 MB, node_exporter ~15–20 MB. The ServerBee Agent's **single-digit MB memory + <1% CPU** is among the lightest.
