...
extension ServerStatus {
    /// Merges non-nil fields from another ServerStatus into this instance.
    /// Used to apply partial updates from WebSocket messages.
...
extension ServerStatus {
    /// Returns the primary IP address (ipv4 preferred, fallback to ipv6).
    ...
    /// Location string combining region and country.
    var location: String? {
        [region, country].compactMap { $0 }.joined(separator: ", ").nilIfEmpty
    }
}
