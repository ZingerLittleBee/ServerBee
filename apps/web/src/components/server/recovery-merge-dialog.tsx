...
import { useRecoveryCandidates, startRecoveryMerge } from '@/hooks/use-api'
...
export function RecoveryMergeDialog({
  currentJob,
  onOpenChange,
  open,
  targetServerId
}: RecoveryMergeDialogProps) {
  ...
  const startMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error instanceof Error ? error.message : t('recovery_merge_failed', { defaultValue: 'Recovery failed' }))
    }
  })
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        {currentJob && (
          <div className="rounded-lg border bg-muted/50 p-3 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>{t('recovery_merge_existing_job', { defaultValue: 'A recovery job is already in progress.' })}</span>
            </div>
          </div>
        )}
        ...
        <div className="rounded-lg border bg-muted/30 p-3 text-sm text-muted-foreground">
          {t('recovery_merge_warning', {
            defaultValue: 'This keeps the original server record, asks the replacement agent to rebind, and continues the recovery flow from there.'
          })}
        </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button disabled={!canSubmit} onClick={() => selectedCandidate && startMutation.mutate(selectedCandidate.server_id)}>
