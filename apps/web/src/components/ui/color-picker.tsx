...
import { Elevated } from '@/lib/fluid/elevated'
import { fontWeights } from '@/lib/fluid/font-weight'
import { useIcon } from '@/lib/fluid/icon-context'
import { shapeMap, useShape } from '@/lib/fluid/shape-context'
...
function linearToSrgb(c: number): number {
  const v = c <= 0.003_130_8 ? 12.92 * c : 1.055 * c ** (1 / 2.4) - 0.055
...
function parseColor(input: string): { r: number; g: number; b: number; a: number } | null {
  ...
  if (hslM) {
    ...
    const sat = parts[1].endsWith('%') ? Number.parseFloat(parts[1]) / 100 : Number.parseFloat(parts[1])
    const l = parts[2].endsWith('%') ? Number.parseFloat(parts[2]) / 100 : Number.parseFloat(parts[2])
    let a = 1
    if (parts[3] !== undefined) {
      a = parts[3].endsWith('%') ? Number.parseFloat(parts[3]) / 100 : Number.parseFloat(parts[3])
    }
...
function SaturationSquare({ h, s, v, onChange }: SaturationSquareProps) {
  ...
  return (
    <div
      ...
      className={cn('relative w-full cursor-none touch-none select-none outline-none', shape.bg)}
...
function FormatDropdown({
  ...
  return (
    ...
    >
      <Menu.Trigger
        className={cn(
          'flex h-9 cursor-pointer items-center justify-between gap-2 bg-transparent px-3 text-[13px] outline-none transition-colors duration-80 hover:bg-hover hover:text-foreground focus-visible:ring-1 focus-visible:ring-[color:var(--focus-ring,#6B97FF)]',
      ...
      <Menu.Portal container={portalContainer ?? undefined}>
        <Menu.Positioner align="start" className="z-[60] outline-none" side="bottom" sideOffset={6}>
          <motion.div
          ...
          >
            <FormatMenuContext.Provider value={menuCtx}>
              <Menu.Popup
                className={cn(
                  `relative flex min-w-[var(--anchor-width)] flex-col gap-0.5 ${menuShape.container} select-none p-1 outline-none`
                )}
                onBlur={(e) => {
              ...
              >
                ...
                <AnimatePresence>
                  {checkedRect && (
                    <motion.div
                      ...
                      className={`absolute ${menuShape.bg} pointer-events-none bg-active`}
                ...
                <AnimatePresence>
                  {activeRect && (
                    <motion.div
                      ...
                      className={`absolute ${menuShape.bg} pointer-events-none bg-hover`}
                ...
                >
                  {FORMATS.map((fmt, i) => (
                    <FormatItem checked={value === fmt} index={i} key={fmt} label={FORMAT_LABELS[fmt]} value={fmt} />
                  ))}
                </Menu.RadioGroup>
              </Menu.Popup>
...
const TextColorInput = forwardRef<HTMLInputElement, ColorInputProps>(
  (
    ...
    return (
      <div
        className={cn(
          'flex h-9 select-none items-center bg-transparent px-2 transition-colors duration-80 focus-within:ring-1 focus-within:ring-[color:var(--focus-ring,#6B97FF)] hover:bg-hover active:bg-active',
          shape.input,
          className
        )}
        style={{ width }}
      >
        {prefix && <span className="mr-1 select-none text-[12px] text-muted-foreground">{prefix}</span>}
        <input
          aria-label={ariaLabel}
          className={cn(
            'min-w-0 flex-1 bg-transparent text-[13px] text-foreground tabular-nums outline-none',
...
const ScrubColorInput = forwardRef<HTMLInputElement, ColorInputProps>(
  (
    ...
    return (
      <NumberField.Root
        className={cn(
          'flex h-9 select-none items-center bg-transparent transition-colors duration-80 focus-within:ring-1 focus-within:ring-[color:var(--focus-ring,#6B97FF)] hover:bg-hover active:bg-active',
          shape.input,
          className
        )}
        format={format}
      ...
      >
        <NumberField.ScrubArea
          className={cn('flex min-w-0 flex-1 items-center self-stretch px-2', !editing && 'cursor-ew-resize')}
...
function EyeDropperButton({ onPick }: { onPick: (hex: string) => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        'flex h-9 cursor-pointer items-center justify-center bg-transparent px-3 text-muted-foreground outline-none transition-colors duration-80 hover:bg-hover hover:text-foreground focus-visible:ring-1 focus-visible:ring-[color:var(--focus-ring,#6B97FF)] active:bg-active',
        shape.input
      )}
      onClick={handleClick}
      type="button"
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {swatches.map((sw, i) => {
        ...
        return <ColorSwatch color={sw} key={`${sw}-${i}`} onClick={() => onPick(sw)} selected={isSelected} size={28} />
      })}
    </div>
  )
}
    ...
    return (
      <SurfaceProvider value={pickerLevel}>
        <div
          ...
          {...props}
        >
          <SaturationSquare h={hsv.h} onChange={(s, v) => updateHsv({ s, v })} s={hsv.s} v={hsv.v} />

          <div className="flex flex-col [&>*+*]:-mt-px [&>*]:mb-0">
            <HueSlider
              h={hsv.h}
              onChange={(h) => {
                oklchHueRef.current = null
          ...
          {swatches && swatches.length > 0 && (
            <SwatchStrip current={parsed.hex} onPick={handleSwatchPick} swatches={swatches} />
          )}
        </div>
      </SurfaceProvider>
    )
  ...
  if (format === 'hex') {
    ...
    return (
      <div className="grid grid-cols-2 gap-2">
        ...
        <AlphaInput onCommit={(n) => onChannelChange('alphaPercent', String(n))} value={alphaPct} />
      </div>
    )
  }
    ...
    return (
      ...
      >
        <div className="inline-flex" ref={ref}>
          <Popover.Trigger
            className={cn(
              'flex h-9 cursor-pointer items-center gap-2 border border-border bg-transparent px-2 outline-none transition-colors duration-80 hover:bg-hover focus-visible:ring-1 focus-visible:ring-[color:var(--focus-ring,#6B97FF)]',
              shape.input,
              triggerClassName
            )}
            style={{ fontVariationSettings: fontWeights.medium }}
          >
            {triggerLabel && triggerLabelPosition === 'left' && (
              <span className="select-none px-1 text-[13px] text-muted-foreground">{triggerLabel}</span>
            )}
            <ColorTile color={swatchColor} size={20} />
            {triggerShowValue && <span className="text-[13px] text-foreground tabular-nums">{valueLabel}</span>}
          ...
          <Popover.Portal>
            <Popover.Positioner align="start" className="z-50 outline-none" side="bottom" sideOffset={6}>
              <motion.div
                animate={open ? { opacity: 1, y: 0, scaleY: 1 } : { opacity: 0, y: -4, scaleY: 0.96 }}
                initial={{ opacity: 0, y: -4, scaleY: 0.96 }}
                onAnimationComplete={() => {
              ...
              >
                <Popover.Popup className="outline-none" render={<div ref={setPanelEl} />}>
