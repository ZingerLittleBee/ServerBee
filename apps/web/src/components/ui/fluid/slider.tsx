...
function nearestStepIndex(v: number, steps: number[]): number {
  ...
  for (let i = 1; i < steps.length; i++) {
    if (Math.abs(steps[i] - v) < Math.abs(steps[idx] - v)) {
      idx = i
    }
  }
  return idx
...
function pixelToValue(
  ...
  if (stepValues) {
    return stepValues[nearestStepIndex(raw, stepValues)]
  }
...
function ValueDisplay({
        ...
        <span className="inline-grid text-[13px]">
          ...
          <span className="col-start-1 row-start-1 flex items-center gap-1">
            ...
            <input
              ...
              className={cn(
                'w-[5ch] border-border border-b bg-transparent text-center text-foreground outline-none',
                shape.input
              )}
              max={max}
              min={min}
  ...
  return (
    <span
      className={cn(
        'inline-grid shrink-0 text-[13px] text-muted-foreground leading-none transition-[font-variation-settings] duration-100',
        'tabular-nums'
      )}
      style={{
...
function TooltipValue({ value, formatValue, motionX }: TooltipValueProps) {
  ...
  return (
    ...
    >
      <span
        className={cn('whitespace-nowrap bg-foreground px-2 py-1 text-[12px] text-background tabular-nums', shape.bg)}
        style={{ fontVariationSettings: fontWeights.medium }}
      >
        {formatValue(value)}
      </span>
...
const Slider = forwardRef<HTMLDivElement, SliderProps>(
  (
    ...
    const stepDotsMaskRange = useTransform([motionX0, motionX1] as MotionValue<number>[], ([x0, x1]) => {
    ...
    const handlePointerDown = useCallback(
      (e: React.PointerEvent<HTMLDivElement>) => {
        ...
        if (layoutWidth <= 0 || trackRect.width <= 0) {
          return
        }
    ...
    return (
      <div
        className={cn(
          'flex w-full touch-none select-none flex-col gap-0 overflow-visible',
          valuePosition === 'left' || valuePosition === 'right' ? 'mb-2 flex-row items-center gap-2' : 'flex-col',
      ...
      >
        ...
        >
          ...
          {showValue && valuePosition === 'tooltip' && (
            <AnimatePresence>
              {isInteracting && (
                <TooltipValue formatValue={formatValue} key="tooltip-0" motionX={motionX0} value={values[0]} />
              )}
              {isInteracting && isRange && values[1] !== undefined && (
                <TooltipValue formatValue={formatValue} key="tooltip-1" motionX={motionX1} value={values[1]} />
              )}
            </AnimatePresence>
          )}
          ...
          >
            ...
            <AnimatePresence>
              {hoverPreview && showHoverTooltip && !isPressed && valuePosition !== 'tooltip' && (
                ...
                >
                  <span
                    className={cn(
                      'whitespace-nowrap bg-foreground px-2 py-1 text-[12px] text-background tabular-nums',
                      shape.bg
                    )}
                    style={{ fontVariationSettings: fontWeights.medium }}
                  >
            ...
            <motion.div
              ...
              className={cn('absolute overflow-hidden rounded-full border border-border', trackClassName)}
              initial={false}
              style={{
                left: TRACK_INSET,
                right: TRACK_INSET,
...
interface SliderComfortableProps
  extends Omit<
    ...
    'onChange' | 'defaultValue' | 'onDrag' | 'onDragStart' | 'onDragEnd' | 'onDragOver' | 'onAnimationStart'
  > {
  disabled?: boolean
  formatValue?: (v: number) => string
  label?: string
...
const SliderComfortable = forwardRef<HTMLDivElement, SliderComfortableProps>(
  (
    ...
    // Fill motion value
    const fillPercent = useMotionValue(max === min ? 0 : Math.max(0, Math.min(1, (value - min) / (max - min))))
    ...
    const pipsHandleLineLeftStyle = useTransform(fillPercent, (p) => `calc(${p * 100}% + ${11 - 24 * p}px)`)
    const pipsMaskStyle = useTransform([fillPercent, zeroOffset] as MotionValue<number>[], ([p, zo]) => {
    ...
    useEffect(() => {
      if (dragging.current || handleDragging.current) {
        return
      }
    ...
    return (
      <div
        ...
        onMouseMove={(e) => {
          if (disabled || dragging.current || handleDragging.current) {
            return
          }
      ...
      >
        ...
        <motion.div
          ...
          className={cn(
            'relative h-8 w-full touch-none select-none overflow-hidden border border-border outline-offset-2',
            variant === 'scrubber' ? 'flex cursor-ew-resize items-center gap-3 px-4' : 'cursor-ew-resize',
            shape.bg,
            disabled && 'pointer-events-none opacity-50',
            className
          )}
        ...
        >
          ...
          {variant === 'pips' && (
            <div aria-hidden className="pointer-events-none absolute inset-0 z-[2] flex items-center px-2">
              {label && <span className="select-none bg-background px-2 text-[13px] text-transparent">{label}</span>}
              <span
                className="ml-auto select-none bg-background px-2 text-[13px] text-transparent tabular-nums"
