...
function resolveXPixel(xScale: (value: Date) => number, value: Date | number | undefined, fallback: number): number {
  if (value == null) {
    return fallback
  }
...
function resolveYPixel(yScale: (value: number) => number, value: number | undefined, fallback: number): number {
  if (value == null) {
    return fallback
  }
  return yScale(value)
...
function clampRectToPlot(rect: ReferenceAreaRect, innerWidth: number, innerHeight: number): ReferenceAreaRect | null {
...
function isFullyInsidePlot(rect: ReferenceAreaRect, innerWidth: number, innerHeight: number): boolean {
  return rect.x >= 0 && rect.y >= 0 && rect.x + rect.width <= innerWidth && rect.y + rect.height <= innerHeight
}

/** Map data bounds to plot pixels for a reference rectangle. */
export function computeReferenceAreaRect(options: ComputeReferenceAreaRectOptions): ReferenceAreaRect | null {
