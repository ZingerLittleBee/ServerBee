...
import React, { memo, type ReactNode, useCallback, useEffect, useMemo, useRef, useState } from 'react'
...
const SVG_COMPONENT_NAMES = new Set(['ChoroplethFeature', 'ChoroplethGraticule', 'ChoroplethTooltip'])

const SVG_COMPONENT_TYPES = new Set([ChoroplethFeatureLayer, ChoroplethGraticuleLayer, ChoroplethTooltipLayer])

function resolveComponentType(type: unknown): unknown {
  if (typeof type === 'object' && type !== null && 'type' in type && (type as { type?: unknown }).type) {
...
const ChoroplethMercatorContent = memo(function ChoroplethMercatorContent({
  ...
  const featurePaths = data.features.map((feature) => mercator.path(feature) ?? null) as (string | null)[]

  const pathGenerator = useCallback((feature: ChoroplethFeature) => mercator.path(feature) ?? undefined, [mercator])
...
function ChoroplethChartInner({
  ...
  const translate = translateProp ?? [innerWidth / 2 + margin.left, innerHeight / 2 + margin.top + 50]
  ...
  return (
    <Mercator center={center} data={data.features} scale={scale} translate={translate as [number, number]}>
      {(mercator) => {
