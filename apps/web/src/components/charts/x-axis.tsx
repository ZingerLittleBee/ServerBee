...
function indicesForTickCount(length: number, tickCount: number): number[] {
  ...
  const rawIndices = Array.from({ length: tickCount }, (_, index) => Math.round((index / (tickCount - 1)) * span))
...
function appendProjectionTailTicks(
  ...
  const endLabel = formatValue(domainEnd)
...
const XAxisInner = memo(function XAxisInner({
  ...
  const resolvedDateLabels = useMemo(
    () => (formatValue ? data.map((point) => formatValue(xAccessor(point))) : dateLabels),
    [data, dateLabels, formatValue, xAccessor]
  )
  const formatDate = formatValue ?? shortDateFmt.format
  ...
  const labelsToShow = useMemo(() => {
    ...
    return dataTicks
  }, [tickMode, xDomain, data, formatDate, resolvedDateLabels, xAccessor, xScale, margin.left, numTicks])

  const isHovering = tooltipData !== null
  const crosshairX = tooltipData ? tooltipData.x + margin.left : null
  const hoveredLabel =
    isHovering && tooltipData
      ? (resolvedDateLabels[tooltipData.index] ?? formatDate(xAccessor(tooltipData.point)))
      : null
