...
export function getSkeletonHeights(count: number, seed = 0, min = HEIGHT_MIN_PCT, max = HEIGHT_MAX_PCT): number[] {
  const range = max - min
...
export function LineLoadingSweep({
  ...
  const silhouette = (
    <>
      {withArea ? (
        <AreaClosed curve={curve} data={points} fill={`url(#${chartId}-area)`} x={getX} y={getY} yScale={yScale} />
      ) : null}
      <LinePath
        curve={curve}
        data={points}
