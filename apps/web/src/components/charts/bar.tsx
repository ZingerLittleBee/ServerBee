...
const BarInner = memo(function BarInner({
  ...
  return (
    <g className={`bar-series-${uniqueId}`}>
      {/** biome-ignore lint/complexity/noExcessiveCognitiveComplexity: bar geometry covers stacked/grouped × vertical/horizontal plus depth trim */}
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          if (stacked && stackOffsets) {
            ...
            x = scale(offset) ?? 0
            // Local fix: the segment spans its own value, so its width is the
            // scaled length (upstream subtracted the offset a second time in
            // pixel space, which drove every segment after the first negative).
            barW = valuePos - (scale(0) ?? 0)
...
export function Bar(props: BarProps) {
  ...
  return <BarInner {...props} bandWidth={bandWidth} barScale={barScale} barXAccessor={barXAccessor} />
}
