...
import { type ChartPhase, type ChartStatus, DEFAULT_CHART_LIFECYCLE, resolveRestingChartPhase } from './chart-phase'
import { BarLoadingSkeleton } from './loading-sweep'
import { extractReferenceAreaConfigs } from './reference-area-config'
import { useScheduledTooltip } from './use-scheduled-tooltip'
import { buildYScalesForLines, getPrimaryYScale, normalizeYAxisId, wrapSingleYScale } from './y-axis-scales'
...
export interface BarChartProps {
  ...
  /** Local extension: upper bound for the auto-sized band width in pixels (Recharts `maxBarSize`). */
  maxBarWidth?: number
...
/**
 * Local extension: largest value an axis has to fit. Stacked series scale to
 * the column total (upstream only scans single values here, unlike the
 * horizontal `valueScale`), otherwise the stack overflows the plot area.
 */
function resolveAxisMax(data: Record<string, unknown>[], dataKeys: string[], stacked: boolean): number {
  let max = 0
  for (const d of data) {
    let stackSum = 0
...
const ChartCore = memo(function ChartCore({
  ...
  const { tooltipData, setTooltipData, scheduleTooltip, clearTooltip } = useScheduledTooltip<TooltipData>()
  ...
  const categoryScale = useMemo(() => {
    ...
    const axisLength = isHorizontal ? innerHeight : innerWidth
    const range: [number, number] = [0, axisLength]
    const scale = scaleBand<string>({
      range,
      domain,
      padding: barGap
    })
    // Local extension: cap the band width so sparse data keeps readable bars
    // (Recharts `maxBarSize`). Widening the band padding keeps every bar
    // centered in its slot, unlike the fixed `barWidth` prop. d3 band layout:
    // bandwidth = size * (1 - padding) / (count + padding).
    if (maxBarWidth != null && domain.length > 0 && scale.bandwidth() > maxBarWidth) {
      const padding = (axisLength - maxBarWidth * domain.length) / (axisLength + maxBarWidth)
      scale.padding(Math.min(0.95, Math.max(barGap, padding)))
    }
    return scale
  }, [innerWidth, innerHeight, data, categoryAccessor, barGap, isHorizontal, maxBarWidth])
  ...
  // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: stacked and grouped domains share one scan
  ...
  const handleMouseMove = useCallback(
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: hover geometry covers stacked/grouped × vertical/horizontal
...
export function BarChart({
  ...
  return (
    <div className={cn('relative w-full overflow-visible', className)} ref={containerRef} style={{ aspectRatio }}>
