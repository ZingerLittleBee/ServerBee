...
/** Horizontal room a category label needs before neighbours collide. */
...
const BarXAxisInner = memo(function BarXAxisInner({
  tickerHalfWidth = 50,
  showAllLabels = false,
  maxLabels: maxLabelsProp,
  formatLabel,
  container
}: BarXAxisProps & { container: HTMLDivElement }) {
  const { margin, tooltipData, barScale, bandWidth, barXAccessor, data, innerWidth } = useChart()
  // Local extension: narrow charts (dashboard widgets) crowd at the upstream
  // default of 12, so fall back to what the plot width actually fits.
  const maxLabels = maxLabelsProp ?? Math.max(3, Math.min(12, Math.floor(innerWidth / LABEL_SLOT_PX)))
  ...
  const labelsToShow = useMemo(() => {
    ...
    const allLabels = data.map((d) => {
      ...
      const bandX = barScale(category) ?? 0
      // Center the label under the bar group
      const x = bandX + bandWidth / 2 + margin.left
      return { label: formatLabel ? formatLabel(category) : category, x }
    })
  ...
  }, [barScale, bandWidth, barXAccessor, data, margin.left, showAllLabels, maxLabels, formatLabel])
