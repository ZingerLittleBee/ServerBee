...
// Served same-origin from `public/` (embedded into the server binary), so the
// map works in intranet/offline deployments where GitHub raw is unreachable.
const WORLD_DATA_URL = `${import.meta.env.BASE_URL}world-countries.json`
...
let globalFetchPromise: Promise<FeatureCollection<Geometry, CountryProperties> | null> | null = null
...
export function WorldDataProvider({ children }: { children: ReactNode }) {
  ...
  return <WorldDataContext.Provider value={{ worldData, isLoading }}>{children}</WorldDataContext.Provider>
}
