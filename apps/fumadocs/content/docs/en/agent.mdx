...
The ServerBee agent is a lightweight Rust binary that runs on each server you want to monitor. It collects system metrics (CPU, memory, disk, network, load, temperature, GPU) and reports them to the central ServerBee server over a persistent WebSocket connection.

## What the Agent Does

- Collects system metrics every 3 seconds (configurable)
...
Download the agent binary for your platform from the [releases page](https://github.com/ZingerBee/ServerBee/releases):
...
```bash
curl -fsSL https://raw.githubusercontent.com/ZingerBee/ServerBee/main/install-agent.sh | bash
```
...
Like the server, all options support `SB_` prefixed environment variables:

```bash
export SB_SERVER_URL="http://your-server-ip:9527"
export SB_TOKEN="your-agent-token"
export SB_COLLECTOR__INTERVAL=5
export SB_COLLECTOR__ENABLE_GPU=true
```
