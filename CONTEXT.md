...
- **Live metrics (实时指标帧)** — the partial projection of a server carried by WS `update` frames: only the fields an agent report can populate (usage, speeds, loads, connection counts). Static facts are absent from the wire; clients keep their cached values when merging (see ADR-0002). _Avoid_: partial status, update payload.
