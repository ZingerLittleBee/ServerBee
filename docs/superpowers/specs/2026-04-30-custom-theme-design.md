...
- 正则匹配后追加**数值范围校验**(正则不容易表达):
  - L(lightness)∈ `[0.0, 1.0]` — **越界拒收,返回 422**
  - H(hue)∈ `[0.0, 360.0]` — **越界拒收,返回 422**
  - α(alpha)数字形式 ∈ `[0.0, 1.0]`,百分号形式 ∈ `[0%, 100%]` — **越界拒收,返回 422**
  - C(chroma)无硬上限。OKLCH 标准上"实际可呈现区间"约 `[0.0, 0.5]`,超过通常被浏览器 gamut 裁剪,但仍是合法语法。**首版不拦截,只在前端编辑器对 `C > 0.5` 显示一个 inline 提示("colors beyond display gamut may render desaturated"),不阻断保存。** 服务端不做 C 越界拒收,避免封死合法但激进的设计选择。
- 缺 key / 多 key / 正则不匹配 / L·H·α 越界 → `AppError::Validation`,message 指明第一个出错的 key 与原因。
...
- 已有公开接口 `GET /api/status/{slug}`(在 `router/api/status_page.rs::public_router()`)的 `PublicStatusPageData` 响应体加一个 `theme: ThemeResolved` 字段。`ThemeResolved` 是带 tag 的 union,对应预设和自定义两种来源,**形态与 §6.2 active-theme 接口的 `theme` 字段完全一致,共用同一个 DTO**:
  ...
  **DTO 字段约定**(双接口共用):`kind` 与 `id` 必填;`name` / `vars_light` / `vars_dark` / `updated_at` 仅在 `kind === "custom"` 时出现,对预设形态缺省。Rust 用带 tag 的 enum 序列化(`#[serde(tag = "kind")]`),TS 侧用 discriminated union(`type ThemeResolved = { kind: 'preset'; id: string } | { kind: 'custom'; id: number; name: string; vars_light: VarMap; vars_dark: VarMap; updated_at: string }`),消费端只需在分支里访问独有字段,不会越界。
...
1. 同步读 `localStorage.active-theme-cache` → 立刻应用,避免首屏白屏闪烁。
2. 异步 `GET /api/settings/active-theme` → 服务端为准 → 写回 `localStorage.active-theme-cache`。

**`localStorage.active-theme-cache` 存的是完整 `ActiveThemeResponse` 的 JSON**(`{ ref, theme: ThemeResolved }`),而**不是裸 ref 字符串**。因为自定义主题的 `vars_light` / `vars_dark` 是首屏渲染就需要的真值,如果只缓存 ref,首次加载等服务端响应回来之前就要么白屏要么先错误地按预设画一遍再切。键名特意叫 `*-cache` 而不是 `*-ref`,避免被误读为裸引用。

服务端响应回来后做 `JSON.stringify` 写回;若新旧 cache 不等,触发一次重新应用。
...
1. 客户端启动时,读 `localStorage.color-theme`(若存在)只用作**首屏防闪的本地兜底**,与新键 `localStorage.active-theme-cache` 并行;然后调 `GET /api/settings/active-theme`,以服务端为准并写入新键 `active-theme-cache`,**不向服务端写**。
