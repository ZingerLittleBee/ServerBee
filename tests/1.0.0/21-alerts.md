...
> ✅ A2/A3-followup re-arm(**原严重阻断,已修复**): 告警首轮 `trigger→resolve` 后即使条件再满足也永久不再触发的缺陷已修复。根因:`mark_triggered` 的 `else` 分支对 `alert_states` 盲 `INSERT`,resolve 后残留同 `(rule_id,server_id)` 行 → 命中 `UNIQUE` 约束(SQLite 2067)→ 评估永久中止。修复:`else` 分支改为 upsert —— 存在旧行则复位(`resolved=false`、`resolved_at=NULL`、`count=1`、刷新时间)而非盲 INSERT。新增回归测试 `alert.rs::test_alert_rearms_after_resolve_cycle`(trigger→resolve→re-trigger 闭环,断言单行且重新 firing);TDD 红/绿通过,server lib 506 全绿、CI clippy 干净。**编排者真机端到端复验(重建二进制后,无维护窗口干扰)**:连续 2 轮 `re-arm触发 → resolve`——round1 触发@resolved:false→resolve@resolved:true、round2 触发@resolved:false(fresh first_triggered_at)→resolve@resolved:true,4/4 子检查 PASS,webhook 每阶段均投递,server 日志 `UNIQUE constraint failed: alert_states` / 评估中止 计数 = **0**。(注:首次复验疑似失败经定位为编排者 A3 创建的活跃维护窗口覆盖同 server 导致评估器正确抑制告警——非 re-arm 缺陷;清理维护窗口后复验 PASS。)**修复确认生效。**
>
...
**汇总**:✅ 5 / ❌ 0 / — 2(A3 恢复通知 + re-arm 缺陷均已修复,真机端到端复验通过——含 2 轮完整状态机闭环且 0 UNIQUE/评估中止)
