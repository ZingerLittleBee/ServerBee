...
| SV-4A | Cleanup 按钮计数只统计离线 orphan | 准备 1 台离线 orphan 和 1 台在线未初始化 server 后进入 `/servers` | 显示 `Clean up unconnected (1)`，不会把在线未初始化 server 算进去 | ✅ agent-browser 实测 |
| SV-4B | Cleanup 操作不误删在线未初始化 server | 点击 cleanup 并确认删除 | cleanup 按钮消失；API 复核仅剩在线 placeholder server | ✅ `GET /api/servers` 仅返回 1 条记录 |
